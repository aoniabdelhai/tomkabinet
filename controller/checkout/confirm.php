...
class ControllerCheckoutConfirm extends Controller { 
	public function index() {
		...
		if (isset($this->session->data['order_id'])) $this->model_checkout_order->cancelOrder($this->session->data['order_id']);
		...
		if (!$redirect) {
			...
			if ($data['store_id']) {
				...
				$data['store_url'] = HTTP_SERVER;	
                        ...
                        //Change sponiza, we cannot use the address, as it is not used in the application
			if ($this->customer->isLogged()) {
				...
				$data['fax'] = $this->session->data['guest']['fax'];
                                ...
                                //Not used, so not defined, and by the way, you cannot buy if you are not logged in
				//$payment_address = $this->session->data['guest']['payment'];
			}

                        //Change sponiza, since the address is not used. Original values below
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				// @edit maarten@rdrd.nl disable booxtream watermark in favor of watermark after payment
				//$securedownloadproducts[] = $product;
			...
			foreach ($securedownloadproducts as $product)
			{
				$this->createSecureDownload($product,
                    $this->customer->getId(),
        ...
        private function createXML($file, $customername, $customeremail, $reference) {
            // set parameters
            $aParams = array(
        ...
        private function send_msg($array, $f) {
   ...
   private function booxstream($file, $customerid, $customername, $customeremail, $reference) {
      $xml = $this->createXML($file, $customername, $customeremail, $reference);
      return $this->send_msg($xml, $file);
   }
   ...
   function createSecureDownload($product, $customerid, $customername, $customeremail, $reference = 'xxx') {
       if (isset($product['download'])) {
          foreach ($product['download'] as $product_download) {
          	 $file = DIR_DOWNLOAD . $product_download['filename'];
             if (substr_compare($product_download['name'], '.epub', strlen($product_download['name'])-strlen('.epub'), strlen('.epub')) === 0) {
                $watermarkfile = $this->booxstream($file, $customerid, $customername, $customeremail, $reference);
                $this->processHash($product['product_id'], $file, $customerid);
