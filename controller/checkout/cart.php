...
class ControllerCheckoutCart extends Controller {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			foreach ($products as $product) {
				...
				if ($product['image']) {
					...
					{
						$image = S3_COVER_URL . 'resized/' . image_resize_name(basename($product['image']), $this->config->get('config_image_cart_width'));
					}
					else {
						$image = $this->model_tool_image->onesize($product['image'], $this->config->get('config_image_cart_width'));
					}
				} else {
				...
				if ($product['recurring']) {
					...
					if ($product['recurring_duration']) {
						...
						$profile_description .= sprintf($this->language->get('text_payment_until_canceled_description'), $recurring_price, $product['recurring_cycle'], $frequencies[$product['recurring_frequency']], $product['recurring_duration']);
                                ...
                                //Change sponiza, vqmod, add the authors and book information
                                $p = $this->model_catalog_product->getProduct($product['product_id']);
                                $c = $this->model_catalog_product->getCategorienames($product['product_id']);
                                $genre = "";
                                $i = 0;
                                foreach ($c as $cat) {
                                    if ($i > 0) {
			...
			$this->data['continue'] = $this->url->link('product/adsattributes');
	...
	public function add() {
		...
		if(isset($this->request->post['productpage'])){
