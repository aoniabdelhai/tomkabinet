...
class ControllerSellerAccountProduct extends ControllerSellerAccount {
	public function getTableData() {
		...
		foreach ($products as $product) {
			...
			// product status
			$status = "";
						//echo 'productstatus ' . $product['mp.product_status'] . ' x ' . MsProduct::STATUS_ACTIVE . ' x ' . ($product['mp.product_status'] == MsProduct::STATUS_ACTIVE);
			if ($product['mp.product_status'] == MsProduct::STATUS_ACTIVE) {
	...
	public function jxUploadDownloads() {
		...
		$this->load->model('catalog/banned');
		$this->load->model('catalog/booklanguage');
		$this->load->model('catalog/titelbank');
		...
		foreach ($_FILES as $file) {
			if ($msconf_downloads_limits[1] > 0 && $this->request->post['fileCount'] >= $msconf_downloads_limits[1]) {
				...
				if ($errors) {
					...
					*/
					//$json['bookinfo'] = $this->MsLoader->MsFileChecker->doAction($fileData);
					$epubData = $this->MsLoader->MsFileChecker->doAction($fileData);
					if (isset($epubData['errorKeys']))
					{
						foreach ($epubData['errorKeys'] as $strErrorKey)
						{
							$json['errors'][] = $this->language->get($strErrorKey);
						}
					}
					else if (!$this->processHash('download/tmp/' . $fileData['fileName'], $this->customer->getId())) {
						 $strErrorKey = 'ms_error_product_uploaded_before';
						 $json['errors'][] = $this->language->get('ms_error_product_uploaded_before') . ' (' . $epubData['authorstring'] . ' - ' . $epubData['title'] . ')';
					}
					else if (true === $this->model_log_auditlog->isBoughtHere(hash_file('sha512', 'download/tmp/' . $fileData['fileName'])))
					{
						$blnBoughtHere = true;
						if (true === $this->model_catalog_banned->isBanned($epubData['authorstring'], $epubData['title']))
						{
							$strErrorKey = 'bought_here_banned';
							$json['warnings'][] = $this->language->get('ms_warning_bought_here_banned');
						}
					}
					else if (true === $this->model_catalog_banned->isBanned($epubData['authorstring'], $epubData['title']))
					{
						$strErrorKey = 'ms_banned_error';
						$json['errors'][] = $this->language->get('ms_banned_error');
					}
					
					if (null === $strErrorKey || 'bought_here_banned' === $strErrorKey || 'multiple_authors' === $strErrorKey)
					{
						$arrBooks = $this->model_catalog_titelbank->getAuthorTitleEbooks($epubData['authorstring'], $epubData['title']);
						if (count($arrBooks) > 1)
						{
							foreach ($arrBooks as $arrBook)
							{
								if ($epubData['isbn'] === $arrBook['Ean'])
								{
									$epubData['isbn'] = $arrBook['Ean'];
									
									// take metadata from Titelbank
									$epubData['book_language'] = $arrBook['Taalvermelding'];
									$epubData['description'] = $arrBook['Samenvatting'];
									
									break;
								}
							}
						}
						else if (false === empty($arrBooks))
						{
							// single result
							$arrBook = $arrBooks[0];
							$epubData['isbn'] = $arrBook['Ean'];
						...
						if (true === empty($arrBooks))
						{
							$arrBooks = $this->model_catalog_titelbank->getTitleEbooks($epubData['title']);
							if (true === empty($arrBooks))
							{
								if (false === $blnBoughtHere)
								{
									$strErrorKey = 'title_not_found';
									$json['errors'][] = $this->language->get('ms_title_not_found_error');
								}
							}
							else if (count($arrBooks) > 1)
							{
								$strAuthor = null;
								foreach ($arrBooks as $arrBook)
								{
									...
									else if (null !== $strAuthor && $strAuthor !== $arrBook['AuteurAchternaam'])
									{
										if (false === $blnBoughtHere)
										{
											unset($json['warnings']);
											$strErrorKey = 'multiple_authors';
											$json['warnings'][] = $this->language->get('ms_warning_multiple_authors');
										}
									}
									
									// AuteurVolledigenaam heeft ook verschillende schrijfwijzen: C. Cussler, Clive Cussler
									$strAuthor = $arrBook['AuteurAchternaam'];
								}
							}
							else
							{
					...
					if (null === $strErrorKey || 'bought_here_banned' === $strErrorKey || 'multiple_authors' === $strErrorKey)
					{
						...
						$strLanguage = $this->model_catalog_booklanguage->getLanguageCodeByLanguageMeta($epubData['book_language']);
						if (false === $blnBoughtHere && $epubData['book_language'] && 'nl' !== $strLanguage)
						{
							$strErrorKey = 'not_nl';
							$json['errors'][] = $this->language->get('ms_not_nl_error');
						}
						$epubData['book_language'] = $strLanguage;
					}
					
					if (null === $strErrorKey || 'bought_here_banned' === $strErrorKey || 'multiple_authors' === $strErrorKey)
					{
						//$filenamestart = time() . '_' . md5(rand()) . '.' . $this->MsLoader->MsSeller->getNickname();
						$filenamestart	= time() . '_' . md5(rand()) . '.' . preg_replace('([^a-zA-Z0-9 ])', '', $epubData['authorstring']) . ' - ' . preg_replace('([^a-zA-Z0-9 ])', '', $epubData['title']);
						//$filename		= $filenamestart . '.epub';
						$filepath		= 'download/tmp/' . $fileData['fileName'];
						
						// 'validate' epub using booxtream
						$sUrl = 'http://service.booxtream.com/booxtream.epub';
						$sUserName = 'tomkabinettest';
						$sKey = 'Oe6GdPVrMk0ah60Xz89jbkUHzepghY';
						$aParams = array(
							'customeremailaddress' => 'info@tomkabinet.nl',
							'customername' => 'tomkabinet',
							'languagecode' => '1043',
							...
							// 'epubfile' => '@'.$filepath,
							// 'epubfile' => curl_file_create($filepath) ,
						...
						$aParams['epubfile'] = function_exists('curl_file_create') ?  curl_file_create($filepath) : '@' . $filepath;
						...
						//if ($nStatus !== 200)
						if ($nStatus !== 200 && CONFIG_MODE !== 'maarten-local')
						{
							 //echo "booxtream error: ".$nStatus . "\n";
							 //echo $sOutput;
							 //die();
							//
							// @log of original xml
							//if (!is_dir('./download/tmp/booxtream/')) mkdir('./download/tmp/booxtream/');
							//$ip = str_replace(':', '%', $this->request->server['REMOTE_ADDR']);
							// file_put_contents('./download/tmp/booxtream/' . $ip . ' - ' . date('Y-m-d H.i.s') . '.xml', $sOutput);
							
							$strErrorKey = 'booxtream_error';
							$json['errors'][] = $this->language->get('ms_watermark_error');
						}
						else
						{
							if (false === $this->model_log_auditlog->testLatestOwnerHash($epubData['hash'], $this->customer->getId()))
							{
								$json['warnings'][] = $this->language->get('ms_warning_product_not_current_owner');
							}
							
							// use cover from TitelBank
							if (!$epubData['image'] && true === EPUB_USE_AMAZON_S3)
							{
								$cover = 'data/EPUB/' . $epubData['isbn'] . '_VRK.jpg';
								if (false !== @S3::getObject(S3_BUCKET, 'titelbank-covers/' . $epubData['isbn'] . '_VRK.jpg', DIR_IMAGE . $cover))
								{
									$epubData['image'] = $cover;
								}
							}
							if (!$epubData['image'] && true === EPUB_USE_AMAZON_S3)
							{
								$cover = 'data/EPUB/' . $epubData['isbnMeta'] . '_VRK.jpg';
								if (false !== @S3::getObject(S3_BUCKET, 'titelbank-covers/' . $epubData['isbnMeta'] . '_VRK.jpg', DIR_IMAGE . $cover))
								{
									$epubData['image'] = $cover;
								}
							}
							...
							/*
							// get extra metadata from Google Books API
							$isbn = false;
							if ($epubData['isbn'])
							{
								$isbn = $epubData['isbn'];
							}
							else if (1 === preg_match('(epubFile=[^&]*?([0-9]{10,13})[^&]*?&)', $fileData['originalFileName'], $arrMatches))
							{
								// get potential ISBN number from filename
								$isbn = $arrMatches[1];
							}
							...
							if (false !== $isbn)
							{
								$googleJson = file_get_contents('https://www.googleapis.com/books/v1/volumes?country=NL&q=isbn:' . $isbn);
								$googleData = json_decode($googleJson, true);
								$googleItem = isset($googleData['items']) ? reset($googleData['items']) : false;
							}

							if (false === $googleItem)
							{
								$googleJson = file_get_contents('https://www.googleapis.com/books/v1/volumes?country=NL&langRestrict=nl&q=inauthor:' . rawurlencode($epubData['authorstring']) . '+intitle:' . rawurlencode($epubData['title']));
								$googleData = json_decode($googleJson, true);
								$googleItem = isset($googleData['items']) ? reset($googleData['items']) : false;
							}
							...
							if (false !== $googleItem)
							{
								if (isset($googleItem['volumeInfo']['publisher'])) $epubData['publisher'] = $googleItem['volumeInfo']['publisher'];
								if (isset($googleItem['volumeInfo']['publishedDate'])) $epubData['date_published'] = $googleItem['volumeInfo']['publishedDate'];
								if (isset($googleItem['volumeInfo']['description'])) $epubData['description'] = $googleItem['volumeInfo']['description'];
								if (isset($googleItem['volumeInfo']['language'])) $epubData['book_language'] = $googleItem['volumeInfo']['language'];
								if (isset($googleItem['volumeInfo']['imageLinks']['thumbnail']))
								{
									if (!$epubData['image'])
									{
										// @note assume google thumbnail is jpg
										$cover = 'data/EPUB/' . $filenamestart . '.jpg';
										file_put_contents(DIR_IMAGE . $cover, fopen($googleItem['volumeInfo']['imageLinks']['thumbnail'], 'r'));
										$epubData['image'] = $cover;
									}
								}

								if (isset($googleItem['volumeInfo']['industryIdentifiers']))
								{
									foreach($googleItem['volumeInfo']['industryIdentifiers'] as $identifierData)
									{
										if ('ISBN_13' === $identifierData['type'] || ('ISBN_10' === $identifierData['type'] && !isset($epubData['isbn'])))
										{
											$epubData['isbn'] = $identifierData['identifier'];
										}
									}
								}
							}
							*/
							...
							if (true === EPUB_USE_AMAZON_S3)
							{
								// save epub, cover image to S3
								
								// epub
								S3::putObject(S3::inputFile($filepath, false) , S3_BUCKET, 'epubs/original/unpublished/' . $fileData['fileName']);
								
								if ($epubData['image'])
								{
									// cover
									S3::putObject(S3::inputFile(DIR_IMAGE . $epubData['image'], false) , S3_BUCKET, 'covers/original/' . basename($epubData['image']) , S3::ACL_PUBLIC_READ);

									// cover in different sizes
									$configSizes = array(
										'config_image_product_',
										'config_image_additional_',
										'config_image_popup_'
									);
									$configMaxSizes = array(
										'config_image_popup_',
										'config_image_thumb_',
										'config_image_cart_'

										// 'config_image_wishlist_',
										// 'config_image_compare_',
										// 'config_image_related_'

									);
									
									//google_base.php:500x500
									//adsattributes.php:40x40
									//bestseller.php
									//carousel.php
									//featured.php
									//slideshow.php
									//admin product.php:40x40
									
									$this->load->model('tool/image');

									// resize
									foreach($configSizes as $configKey)
									{
										$width = $this->config->get($configKey . 'width');
										$height = $this->config->get($configKey . 'height');
										$image_url = $this->model_tool_image->resize($epubData['image'], $width, $height);
										if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1')))
										{
											$image_url = str_replace($this->config->get('config_ssl') , '', $image_url);
										}
										else
										{
											$image_url = str_replace($this->config->get('config_url') , '', $image_url);
										}

										S3::putObject(S3::inputFile($image_url, false) , S3_BUCKET, 'covers/resized/' . basename($image_url) , S3::ACL_PUBLIC_READ);
									}

									// onesize
									foreach($configMaxSizes as $configKey)
									{
										$width = $this->config->get($configKey . 'width');
										$image_url = $this->model_tool_image->onesize($epubData['image'], $width);
										if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1')))
										{
											$image_url = str_replace($this->config->get('config_ssl') , '', $image_url);
										}
									...
									// hardcoded other width with no 'a' type (probably doesn't use resize() function in site, but other function)
									$width = 40;
									$height = 40;
									$image_url = $this->model_tool_image->resize($epubData['image'], $width, $height);
									if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1')))
									...
									// remove 'a' type from filename
									$intExtensionPos = strrpos($image_url, '.');
									$strExtension = substr($image_url, $intExtensionPos + 1);
									$strFile = substr($image_url, 0, $intExtensionPos - 1) . '.' . $strExtension;
									rename($image_url, $strFile);
									$image_url = $strFile;
									S3::putObject(S3::inputFile($image_url, false) , S3_BUCKET, 'covers/resized/' . basename($image_url) , S3::ACL_PUBLIC_READ);
									
									// datafile to S3 maybe not necessary because it's sort of session based data
								}
							}
							
							$epubData['errorKey'] = $strErrorKey;
							file_put_contents($epubData['datafile'], json_encode($epubData));
							
							$json['title'] = $epubData['title'];
							$json['authorstring'] = $epubData['authorstring'];
							$json['date_published'] = $epubData['date_published'];
							$json['description'] = $epubData['description'];
							$json['book_language'] = $epubData['book_language'];
							$json['isbn'] = $epubData['isbn'];
							$json['image'] = $epubData['image'];
							if ($json['image']) $json['image'] = 'image/' . $json['image'];
							$json['datafile'] = $epubData['datafile'];
							if (true === EPUB_USE_AMAZON_S3 && $json['image'])
							{
								$json['image'] = S3_COVER_URL . 'original/' . basename($json['image']);
							}
						}
					}
					...
					if (null !== $strErrorKey && 'bought_here_banned' !== $strErrorKey && 'multiple_authors' !== $strErrorKey)
					{
						$this->load->model('catalog/refused');
						
						$this->model_catalog_refused->addRefused($fileData['fileName'], $strErrorKey);
					}
				}
			}
		}
	...
	/*
	public function jxGetEpubData()
	{
		$json = array();
		if (!isset($this->request->post['webserviceResponse']))
		{
		...
		else
		{
			...
			// @note opencart automatically htmlencodes the post variables?????????

			$webserviceData = json_decode(html_entity_decode($this->request->post['webserviceResponse']) , true);
			...
			else
			{
				...
				$bookId = null;
				$bookName = null;
				foreach($webserviceData as $bookId => $bookName)
				{
				...
				else
				{
					...
					// request epub data from validator API/webservice

					$epubJson = file_get_contents('http://api.tomkabinet.nl/ValidateEpub/getBookMetaData?bookId=' . $bookId . '&bookName=' . $bookName);

					//
					// @log of original json

					if (!is_dir('./download/tmp/validator/')) mkdir('./download/tmp/validator/');
					$ip = str_replace(':', '%', $this->request->server['REMOTE_ADDR']);

					// file_put_contents('./download/tmp/validator/' . $ip . ' - ' . date('Y-m-d H.i.s') . '.json', $epubJson);
					//
					// echo microtime(true) - $s . "\n";

					if ($epubJson === '')
					{
						$json['errors'][] = $this->language->get('ms_validation_error');
					}
					else
					{
						$epubData = json_decode($epubJson, true);

						// echo microtime(true) - $s . "\n";
						// fix string nulls i.e. bookCoverUrl:"null"

						foreach($epubData as $key => $value)
						{
							if ($value === 'null') $epubData[$key] = null;
						}

						if (!isset($epubData['bookTitle']) || !$epubData['bookTitle'])
						{
							$json['errors'][] = $this->language->get('ms_validation_error');
						}

						// @note validation check disabled until webservice doesn't invalidate valid epubs
						// else if (!isset($epubData['validationSuccessful']) || !$epubData['validationSuccessful'])
						...
						{
							$this->load->model('catalog/banned');
							if (true === $this->model_catalog_banned->isBanned($epubData['author'], $epubData['bookTitle']))
							{
								$json['errors'][] = $this->language->get('ms_banned_error');
							}
							else
							{
								...
								// use ->testHash() instead of ->processhash() because we	have the hash instead of the file

								if (!$this->model_log_auditlog->testHash($epubData['hash'], $this->customer->getId()))
								{
									$json['errors'][] = $this->language->get('ms_error_product_uploaded_before') . ' (' . $epubData['author'] . ' - ' . $epubData['bookTitle'] . ')';
								}
								else
								{
									...
									if (!$this->model_log_auditlog->testLatestOwnerHash($epubData['hash'], $this->customer->getId()))
									{
										$json['warnings'][] = $this->language->get('ms_warning_product_not_current_owner');
									}
									...
									$filenamestart = time() . '_' . md5(rand()) . '.' . preg_replace('([^a-zA-Z0-9 ])', '', $epubData['author']) . ' - ' . preg_replace('([^a-zA-Z0-9 ])', '', $epubData['bookTitle']);
									$cover = null;
									if ($epubData['bookCoverURL'])
									{

										// download & save book cover

										$epubData['bookCoverURL'] = str_replace('localhost:8080', 'http://api.tomkabinet.nl/ValidateEpub', $epubData['bookCoverURL']); // fix api url
										$extension = 'jpg';
										parse_str(parse_url($epubData['bookCoverURL'], PHP_URL_QUERY) , $arr);
										if (isset($arr['coverName']) && (false !== $pos = strrpos($arr['coverName'], '.')))
										{
											$extension = substr($arr['coverName'], $pos + 1);
										}

										$cover = 'data/EPUB/' . $filenamestart . '.' . $extension;
										file_put_contents(DIR_IMAGE . $cover, fopen($epubData['bookCoverURL'], 'r'));
									}

									// echo microtime(true) - $s . "\n";
									// download & save epub

									$epubData['downloadURL'] = str_replace('localhost:8080', 'http://api.tomkabinet.nl/ValidateEpub', $epubData['downloadURL']); // fix api url
									$filename = $filenamestart . '.epub';
									$filepath = DIR_DOWNLOAD . $this->config->get('msconf_temp_download_path') . $filename;
									file_put_contents($filepath, fopen($epubData['downloadURL'], 'r'));

									// save backup of original epub upload

									if (!is_dir(DIR_DOWNLOAD . $this->config->get('msconf_temp_download_path') . 'original/')) mkdir(DIR_DOWNLOAD . $this->config->get('msconf_temp_download_path') . 'original/');
									copy($filepath, DIR_DOWNLOAD . $this->config->get('msconf_temp_download_path') . 'original/' . $filename);
									...
									else
									{
										...
										// map API fields to own fields

										$jsonData = array();
										$jsonData['datafile'] = "tmp/details/" . uniqid() . '_' . preg_replace('([^a-zA-Z0-9 ])', '', $epubData['author']) . ' - ' . preg_replace('([^a-zA-Z0-9 ])', '', $epubData['bookTitle']) . '.json';
										$arrDownload = array(
											'filename' => $filename,
											'hash' => $epubData['hash']
										);
										$jsonData['product_downloads'][] = $arrDownload;
										$jsonData['uuid'] = $epubData['uuid'];
										$jsonData['title'] = $epubData['bookTitle'];
										$jsonData['authorstring'] = $epubData['author'];
										$jsonData['publisher'] = $epubData['publisher'];
										$jsonData['date_published'] = $epubData['publishYear'];
										$jsonData['description'] = $epubData['description'];
										$jsonData['book_language'] = $epubData['language'];
										$jsonData['isbn'] = '';
										$jsonData['image'] = $cover;
										$jsonData['hash'] = $epubData['hash'];
										if (true)
										{
											...
											$googleItem = false;
											if (1 === preg_match('(epubFile=[^&]*?([0-9]{10,13})[^&]*?&)', $epubData['downloadURL'], $arrMatches))
											{
												$googleJson = file_get_contents('https://www.googleapis.com/books/v1/volumes?q=isbn:' . $arrMatches[1]);

												// @log of original json

												if (!is_dir('./download/tmp/google/')) mkdir('./download/tmp/google/');
												$ip = str_replace(':', '%', $this->request->server['REMOTE_ADDR']);

												// file_put_contents('./download/tmp/google/' . $ip . ' - ' . date('Y-m-d H.i.s') . '.json', 'https://www.googleapis.com/books/v1/volumes?q=isbn:' . $arrMatches[1] . "\n" . $googleJson, FILE_APPEND);
											...
											if (false === $googleItem)
											{
												$googleJson = file_get_contents('https://www.googleapis.com/books/v1/volumes?langRestrict=nl&q=inauthor:' . rawurlencode($jsonData['authorstring']) . '+intitle:' . rawurlencode($jsonData['title']));
												...
												// file_put_contents('./download/tmp/google/' . $ip . ' - ' . date('Y-m-d H.i.s') . '.json', 'https://www.googleapis.com/books/v1/volumes?langRestrict=nl&q=inauthor:' . rawurlencode($jsonData['authorstring']) . '+intitle:' . rawurlencode($jsonData['title']) . "\n" . $googleJson, FILE_APPEND);
											...
											if (false !== $googleItem)
											{
												...
												// echo microtime(true) - $s . "\n";
												// $jsonData['title'] = $googleItem['volumeInfo']['title'];
												// $jsonData['authorstring'] = implode(', ', $googleItem['volumeInfo']['authors']);

												if (isset($googleItem['volumeInfo']['publisher'])) $jsonData['publisher'] = $googleItem['volumeInfo']['publisher'];
												if (isset($googleItem['volumeInfo']['publishedDate'])) $jsonData['date_published'] = $googleItem['volumeInfo']['publishedDate'];
												if (isset($googleItem['volumeInfo']['description'])) $jsonData['description'] = $googleItem['volumeInfo']['description'];
												if (isset($googleItem['volumeInfo']['language'])) $jsonData['book_language'] = $googleItem['volumeInfo']['language'];
												if (isset($googleItem['volumeInfo']['imageLinks']['thumbnail']))
												{
												...
												if (isset($googleItem['volumeInfo']['industryIdentifiers']))
												{
													foreach($googleItem['volumeInfo']['industryIdentifiers'] as $identifierData)
													{
														if ('ISBN_13' === $identifierData['type'] || ('ISBN_10' === $identifierData['type'] && !isset($jsonData['isbn'])))
														{
															$jsonData['isbn'] = $identifierData['identifier'];
														}
													}
												}
											}
										...
										file_put_contents('./download/' . $jsonData['datafile'], json_encode($jsonData));
										if (true === EPUB_USE_AMAZON_S3)
										{

											// save epub, cover image & datafile to S3
											...
											S3::putObject(S3::inputFile($filepath, false) , S3_BUCKET, 'epubs/original/unpublished/' . $filename);
											if (null !== $jsonData['image'])
											{

												// cover

												S3::putObject(S3::inputFile(DIR_IMAGE . $jsonData['image'], false) , S3_BUCKET, 'covers/original/' . basename($jsonData['image']) , S3::ACL_PUBLIC_READ);
												...
												foreach($configSizes as $configKey)
												{
													$width = $this->config->get($configKey . 'width');
													$height = $this->config->get($configKey . 'height');
													$image_url = $this->model_tool_image->resize($jsonData['image'], $width, $height);
													if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1')))
													{
														$image_url = str_replace($this->config->get('config_ssl') , '', $image_url);
													}
												...
												{
													$width = $this->config->get($configKey . 'width');
													$image_url = $this->model_tool_image->onesize($jsonData['image'], $width);
													if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1')))
													{
														$image_url = str_replace($this->config->get('config_ssl') , '', $image_url);
													}
												...
												// datafile maybe not necessary because it's sort of session based data
										...
										if (!in_array($filename, $this->session->data['multiseller']['files'])) $this->session->data['multiseller']['files'][] = $filename;
										$json['title'] = $jsonData['title'];
										$json['authorstring'] = $jsonData['authorstring'];
										$json['date_published'] = $jsonData['date_published'];
										$json['description'] = $jsonData['description'];
										$json['book_language'] = $jsonData['book_language'];
										$json['isbn'] = $jsonData['isbn'];
										$json['image'] = $jsonData['image'];
										if (null !== $json['image']) $json['image'] = 'image/' . $json['image'];
										$json['datafile'] = './download/' . $jsonData['datafile'];
										if (true === EPUB_USE_AMAZON_S3 && null !== $json['image'])
										{
											$json['image'] = S3_COVER_URL . 'original/' . basename($json['image']);
										}
									}
	...
	*/
	...
	public function jxSubmitProduct() {
		...
		if (!isset($data['product_downloads'])) {
						//Change sponiza, vqmod
			if ($msconf_downloads_limits[0] > 0 && $this->MsLoader->MsProduct->getNumProductDownloads($data['product_id']) == 0) {
			...
			if (isset($data['product_id']) && !empty($data['product_id'])) {
				...
				$fee = (float)$commissions[MsCommission::RATE_LISTING]['flat'] + $commissions[MsCommission::RATE_LISTING]['percent'] * $data['product_price'] / 100;
								...
								$product_id = $this->MsLoader->MsProduct->saveProduct($data);
								 	/*
								$tmpdownloads = $this->MsLoader->MsProduct->getProductDownloads($product_id);
								foreach ($tmpdownloads as $key => $download) {
									 if (!empty($download['filename'])) {
										$this->insertHash($product_id, DIR_DOWNLOAD . $download['filename'], $this->customer->getId());
									 }
								}
								*/
								$this->load->model('log/auditlog');
								foreach ($data['product_downloads'] as $key => $download) {
									$this->model_log_auditlog->createHash($product_id, $download['hash'], $this->customer->getId());
								}
				...
				$this->load->model('catalog/check');
				$this->model_catalog_check->addChecked($product_id, $data);
	...
	private function _initForm() {
		...
		if (!empty($attributes)) {
			foreach ($attributes as $attr) {
				...
				foreach ($attr['values'] as &$value) { $value['image'] = (!empty($value['image']) ? $this->MsLoader->MsFile->resizeImage($value['image'], 50, 50) : $this->MsLoader->MsFile->resizeImage('no_image.jpg', 50, 50));
				}
	...
	public function update() {
		/* sponiza edit */
	//	$this->document->addScript('catalog/view/javascript/account-product-helper.js');
	 //	 $data_tab_fields = $this->get_ebook_fields();
	 //	 $this->config->set("msconf_product_included_fields", $data_tab_fields);
		/*****************/
		...
		$product['product_cover_images'] = $this->MsLoader->MsProduct->getProductImages($product_id);
		foreach($product['product_cover_images'] as &$img)
		{
			if (EPUB_USE_AMAZON_S3)
			{
				$img['image'] = S3_COVER_URL . 'original/' . basename($img['image']);
			}
			else
			{
	...
	public function download() {
		...
		if ($download_info) {
			...
			if (!headers_sent()) {
				...
				if (true === EPUB_USE_AMAZON_S3)
				{
					...
					$this->MsLoader->MsProduct->deleteProduct($product_id,$seller_id);
		...
		private function processHash($file, $sellerid) {
				$this->load->model('log/auditlog');
				//1. calculate hash
				$hashstring = hash_file("sha512",$file);
				//2. store hash auditlog
				return $this->model_log_auditlog->testHash($hashstring, $sellerid);
		}

		private function insertHash($pid, $file, $sellerid) {
				$this->load->model('log/auditlog');
				//1. calculate hash
				$hashstring = hash_file("sha512",$file);
				//2. store hash auditlog
				return $this->model_log_auditlog->createHash($pid, $hashstring, $sellerid);
		}
