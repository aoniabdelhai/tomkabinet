<?php 
require_once(DIR_SYSTEM . 'helper/iban/iban.php');
	...
	private $error = array();
  	...
  	public function index() {
		...
		if ($this->customer->isLogged()) {
	  		$this->redirect($this->url->link('seller/account-product/create', '', 'SSL'));
    	}
		...
		$this->data['button_continue'] = $this->language->get('button_create');
		...
		if (isset($this->error['seller_nickname'])) {
			...
			$this->data['error_seller_nickname'] = '';
                ...
                //Change sponiza
		if (isset($this->error['seller_legal'])) {
			$this->data['error_seller_legal'] = $this->error['seller_legal'];
		} else {
			$this->data['error_seller_legal'] = '';
		}
  	...
  	protected function validate() {
    	...
    	*/
    	...
    	if (!preg_match('/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/', $this->request->post['password'])) {
    	   $this->error['password'] = $this->language->get('error_password');
    	}
		...
		if (mb_strlen($data['seller_description']) > 1000) {
			...
			$this->error['seller_description'] = $this->language->get('ms_error_sellerinfo_description_length');
                ...
                // if ($this->request->post['seller_paypal'] == '') {
                if (!iban($this->request->post['seller_paypal'])) {
			$this->error['seller_paypal'] = $this->language->get('ms_error_sellerinfo_paypal');
		}
		
                //Change sponiza en mage removed
                if (!isset($this->request->post['seller_approved'])) {
			//$this->error['seller_legal'] = $this->language->get('ms_error_sellerinfo_legal');
                }
