...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		if (!isset($this->request->get['path'])) {
			$this->request->get['path'] = $this->model_catalog_product->getFirstCatagory($product_id);
		}
		...
		if ($product_info) {
			...
			if ($product_info['image']) {
				...
				{
					$this->data['popup'] = S3_COVER_URL . 'resized/' . image_resize_name(basename($product_info['image']), $this->config->get('config_image_popup_width'));
				}
				else
				{
					$this->data['popup'] = $this->model_tool_image->onesize($product_info['image'], $this->config->get('config_image_popup_width'));
				}
			} else {
				$this->data['popup'] = '';
			}
			...
			if ($product_info['image']) {
				...
				if (EPUB_USE_AMAZON_S3)
				{
					$this->data['thumb'] = S3_COVER_URL . 'resized/' . image_resize_name(basename($product_info['image']), $this->config->get('config_image_thumb_width'));
				}
				else
				{
				...
				$this->data['thumb'] = $this->model_tool_image->onesize("data/noimage.jpg", $this->config->get('config_image_thumb_width'));
			}
			...
			$this->data['other_products'] = $this->language->get('text_hasresults');
			
			$data = array(
				'filter_category_id' => $category_id,
				'not'		=> $this->request->get['product_id'],
				'sort'      => 'RAND()',
				'quantity'	=> 10
			);
			...
			if(count($results)<1){
				...
				$this->data['other_products'] = $this->language->get('text_noresults');
			...
			foreach ($results as $result) {
				...
				if ($result['image']) {
					$image = $this->model_tool_image->onesize($result['image'], $this->config->get('config_image_related_width'));
				} else {
					$image = $this->model_tool_image->onesize("data/noimage.jpg", $this->config->get('config_image_related_width'));
				}
